...
CREATE TABLE IF NOT EXISTS TYPES_OF_REPORTS (
    pk_id_type_of_report SERIAL PRIMARY KEY,
    type VARCHAR(100) NOT NULL UNIQUE
);

CREATE TABLE IF NOT EXISTS REPORTS_PROFILE_USERS (
    pk_id_report SERIAL PRIMARY KEY,
    description TEXT NOT NULL,
    fk_id_type_of_report INTEGER NOT NULL,
    fk_id_profile_user INTEGER NOT NULL,
    FOREIGN KEY (fk_id_type_of_report) REFERENCES TYPES_OF_REPORTS (pk_id_type_of_report) ON DELETE RESTRICT ON UPDATE CASCADE,
    FOREIGN KEY (fk_id_profile_user) REFERENCES PROFILES_USERS (pk_id_profile_user) ON DELETE CASCADE ON UPDATE CASCADE
);

CREATE TABLE IF NOT EXISTS TYPES_OF_REPORTS_EVENTS (
    pk_id_type_of_report_event SERIAL PRIMARY KEY,
    type VARCHAR(100) NOT NULL UNIQUE
);

CREATE TABLE IF NOT EXISTS REPORTS_EVENTS (
    pk_id_report_event SERIAL PRIMARY KEY,
    description TEXT NOT NULL,
    fk_id_type_of_report_event INTEGER NOT NULL,
...
-- Asegurar que el usuario tenga acceso a las nuevas tablas
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO chinanko_user;
GRANT USAGE, SELECT, UPDATE ON ALL SEQUENCES IN SCHEMA public TO chinanko_user;
